...
Partial Class Frm_ContrainteListing
...
#Region "ATTRIBUTS"
    ...
    Private Const Nom_page As String = "PAGE-LISTING-CONTRAINTE"  ' POUR LA PAGE
    Private Const Btn_Save As String = "Bouton-SAVE-CONTRAINTE"       ' POUR LE BOUTON D'ENREGISTREMENT
    Private Const Btn_Edit As String = "Bouton-EDIT-CONTRAINTE"       ' POUR LE BOUTON DE MODIFICATION
    Private Const Btn_Delete As String = "Bouton-DELETE-CONTRAINTE"   ' POUR LE BOUTON DE SUPPRESSION
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Is_Acces_Page Then
            If Not IsPostBack Then
                ...
                Btn_ADD_Contrainte.Attributes.Add("onclick", "javascript:void(ShowAddUpdateForm('Frm_ContrainteADD.aspx?" & [Global].ACTION & "=" & [Global].HideMenuHeader & "', 950, 650)); return false;")
                'Btn_ADD_Contrainte.Attributes.Add("onclick", "javascript:Open_Window('Frm_ContrainteADD.aspx', '_self',500,400); return false;") 
                BindGrid()
            End If
        End If
    End Sub
...
#Region "SECURITE"
    Public Sub SYSTEME_SECURITE()
        Try
            ...
            'CType(Page.Master.FindControl("li_Contrainte"), HtmlControl).Attributes.Add("class", "active ")
            'CType(Page.Master.FindControl("i_Contrainte"), HtmlControl).Attributes.Add("class", "fa fa-folder-open fa-lg ")
            ...
            CType(Page.Master.FindControl("li_ContrainteListing"), HtmlControl).Attributes.Add("class", "active")
            ...
            If Session([Global].GLOBAL_SESSION) IsNot Nothing Then
                ...
                Else    ' SI L'UTILISATEUR A ACCES A LA PAGE ON VERIFIE POUR LES BOUTONS ET LES LIENS
                    ...
                    Btn_ADD_Contrainte.Visible = _check
                    rdgContrainte.MasterTableView.Columns.FindByUniqueNameSafe("editer").Visible = _check
                    If Request.QueryString([Global].ACTION) IsNot Nothing Then
                    ...
                    rdgContrainte.MasterTableView.Columns.FindByUniqueNameSafe("delete").Visible = _check
                End If
            End If
...
#Region "Load DATA"
    Private Sub BindGrid(Optional ByVal _refresh As Boolean = True)
        Dim objs As List(Of Cls_Contrainte)
        Dim _ret As Long = 0
        Try
            objs = Cls_Contrainte.SearchAll
            rdgContrainte.DataSource = objs
            If _refresh Then
                rdgContrainte.DataBind()
            End If
            _ret = objs.Count
...
#Region "RADGRID EVENTS"
    Protected Sub rdgContrainte_ItemCommand(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridCommandEventArgs) Handles rdgContrainte.ItemCommand
        Try
            If e.CommandName = Telerik.Web.UI.RadGrid.ExportToExcelCommandName Then
                rdgContrainte.ExportSettings.ExportOnlyData = True
                rdgContrainte.GridLines = GridLines.Both
                rdgContrainte.ExportSettings.IgnorePaging = True
                rdgContrainte.ExportSettings.OpenInNewWindow = False
                rdgContrainte.ExportSettings.FileName = "Liste des Contrainte"
                rdgContrainte.MasterTableView.Columns(0).Visible = False
                rdgContrainte.MasterTableView.ExportToExcel()
            End If
    ...
    Protected Sub rdgContrainte_ItemDataBound(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridItemEventArgs) Handles rdgContrainte.ItemDataBound
        Try
            ...
            If (gridDataItem IsNot Nothing) Then
                ...
                CType(item.FindControl("lbOrder"), Label).Text = rdgContrainte.PageSize * rdgContrainte.CurrentPageIndex + (item.RowIndex / 2)
                ...
                imageediter.Attributes.Add("onclick", "javascript:void(ShowAddUpdateForm('Frm_ContrainteADD.aspx?ID=" & CType(DataBinder.Eval(e.Item.DataItem, "ID"), Long) & "&" & [Global].ACTION & "=" & [Global].HideMenuHeader & "',900,650));")
                REM Privilege
    ...
    Protected Sub rdgContrainte_NeedDataSource(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles rdgContrainte.NeedDataSource
        If IsPostBack Then
            BindGrid(False)
        End If
    End Sub
    ...
    Protected Sub rbtnClearFilters_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles rbtnClearFilters.Click
        Try
            For Each column As GridColumn In rdgContrainte.MasterTableView.Columns
                column.CurrentFilterFunction = GridKnownFunction.NoFilter
                column.CurrentFilterValue = String.Empty
            Next
            rdgContrainte.MasterTableView.FilterExpression = String.Empty
            rdgContrainte.MasterTableView.Rebind()
        Catch ex As Threading.ThreadAbortException
        Catch ex As Rezo509Exception
            MessageToShow(ex.Message)
        Catch ex As Exception
