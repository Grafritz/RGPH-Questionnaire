...
Partial Class Frm_QuestionsListing
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Is_Acces_Page Then
            If Not IsPostBack Then
                ...
                Btn_ADD_Questions.Attributes.Add("onclick", "javascript:void(ShowAddUpdateFormMaximized('Frm_QuestionsADD.aspx?" & [Global].ACTION & "=" & [Global].HideMenuHeader & "', 950, 650)); return false;")
...
#Region "Load DATA"
    ...
    Private Sub BindGrid(Optional ByVal _refresh As Boolean = True)
        ...
        Dim _ret As Long = 0
        Dim IDRubrique As Long = TypeSafeConversion.NullSafeLong(DDL_TypeModule.SelectedValue)
        Try
            objs = Cls_Questions.SearchAll(IDRubrique)
...
#Region "EVENTS CONTROLS"
    Private Sub DDL_TypeModule_SelectedIndexChanged(sender As Object, e As EventArgs) Handles DDL_TypeModule.SelectedIndexChanged
        BindGrid()
    End Sub

    Private Sub CBX_DetailsQuestion_CheckedChanged(sender As Object, e As EventArgs) Handles CBX_DetailsQuestion.CheckedChanged
        rdgQuestions.MasterTableView.Columns.FindByUniqueNameSafe("DetailsQuestion").Visible = CBX_DetailsQuestion.Checked
        rdgQuestions.Rebind()
    End Sub

    Private Sub CBX_CodeCategorie_CheckedChanged(sender As Object, e As EventArgs) Handles CBX_CodeCategorie.CheckedChanged
        rdgQuestions.MasterTableView.Columns.FindByUniqueNameSafe("CodeCategorie").Visible = CBX_CodeCategorie.Checked
        rdgQuestions.Rebind()
    End Sub

    Private Sub CBX_IndicationsQuestion_CheckedChanged(sender As Object, e As EventArgs) Handles CBX_IndicationsQuestion.CheckedChanged
        rdgQuestions.MasterTableView.Columns.FindByUniqueNameSafe("IndicationsQuestion").Visible = CBX_IndicationsQuestion.Checked
        rdgQuestions.Rebind()
    End Sub

    Private Sub CBX_CaratereAccepte_CheckedChanged(sender As Object, e As EventArgs) Handles CBX_CaratereAccepte.CheckedChanged
        rdgQuestions.MasterTableView.Columns.FindByUniqueNameSafe("CaratereAccepte").Visible = CBX_CaratereAccepte.Checked
        rdgQuestions.Rebind()
    End Sub

    Private Sub CBX_NbreCaratereMaximal_CheckedChanged(sender As Object, e As EventArgs) Handles CBX_NbreCaratereMaximal.CheckedChanged
        rdgQuestions.MasterTableView.Columns.FindByUniqueNameSafe("NbreCaratereMaximal").Visible = CBX_NbreCaratereMaximal.Checked
        rdgQuestions.Rebind()
    End Sub

    Private Sub CBX_NbreValeurMinimal_CheckedChanged(sender As Object, e As EventArgs) Handles CBX_NbreValeurMinimal.CheckedChanged
        rdgQuestions.MasterTableView.Columns.FindByUniqueNameSafe("NbreValeurMinimal").Visible = CBX_NbreValeurMinimal.Checked
        rdgQuestions.Rebind()
    End Sub
    ...
    Protected Sub rdgQuestions_ItemDataBound(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridItemEventArgs) Handles rdgQuestions.ItemDataBound
        Try
            ...
            If (gridDataItem IsNot Nothing) Then
                ...
                imageediter.Attributes.Add("onclick", "javascript:void(ShowAddUpdateFormMaximized('Frm_QuestionsADD.aspx?ID=" & CType(DataBinder.Eval(e.Item.DataItem, "ID"), Long) & "&" & [Global].ACTION & "=" & [Global].HideMenuHeader & "',900,650));")
                REM Privilege
