...
REM  Class Frm_CategorieQuestion
...
Partial Class Frm_CategorieQuestionListing
...
#Region "ATTRIBUTS"
    ...
    Private Const Nom_page As String = "PAGE-LISTING-CATEGORIES-QUESTIONS"  ' POUR LA PAGE
    Private Const Btn_Save As String = "Bouton-SAVE-CATEGORIES-QUESTIONS"       ' POUR LE BOUTON D'ENREGISTREMENT
    Private Const Btn_Edit As String = "Bouton-EDIT-CATEGORIES-QUESTIONS"       ' POUR LE BOUTON DE MODIFICATION
    Private Const Btn_Delete As String = "Bouton-DELETE-CATEGORIES-QUESTIONS"   ' POUR LE BOUTON DE SUPPRESSION
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Is_Acces_Page Then
            If Not IsPostBack Then
                ...
                Btn_ADD_CategorieQuestion.Attributes.Add("onclick", "javascript:void(ShowAddUpdateForm('Frm_CategorieQuestionADD.aspx?" & [Global].ACTION & "=" & [Global].HideMenuHeader & "', 950, 650)); return false;")
                'Btn_ADD_CategorieQuestion.Attributes.Add("onclick", "javascript:Open_Window('Frm_CategorieQuestionADD.aspx', '_self',500,400); return false;") 
                BindGrid()
            End If
        End If
    End Sub
...
#Region "SECURITE"
    Public Sub SYSTEME_SECURITE()
        Try
            ...
            'CType(Page.Master.FindControl("li_CategorieQuestion"), HtmlControl).Attributes.Add("class", "active ")
            'CType(Page.Master.FindControl("i_CategorieQuestion"), HtmlControl).Attributes.Add("class", "fa fa-folder-open fa-lg ")
            ...
            CType(Page.Master.FindControl("li_CategorieQuestionListing"), HtmlControl).Attributes.Add("class", "active")
            ...
            If Session([Global].GLOBAL_SESSION) IsNot Nothing Then
                ...
                Else    ' SI L'UTILISATEUR A ACCES A LA PAGE ON VERIFIE POUR LES BOUTONS ET LES LIENS
                    ...
                    Btn_ADD_CategorieQuestion.Visible = _check
                    rdgCategorieQuestion.MasterTableView.Columns.FindByUniqueNameSafe("editer").Visible = _check
                    If Request.QueryString([Global].ACTION) IsNot Nothing Then
                    ...
                    rdgCategorieQuestion.MasterTableView.Columns.FindByUniqueNameSafe("delete").Visible = _check
                End If
            End If
...
#Region "Load DATA"
    Private Sub BindGrid(Optional ByVal _refresh As Boolean = True)
        Dim objs As List(Of Cls_CategorieQuestion)
        Dim _ret As Long = 0
        Try
            objs = Cls_CategorieQuestion.SearchAll
            rdgCategorieQuestion.DataSource = objs
            If _refresh Then
                rdgCategorieQuestion.DataBind()
            End If
            _ret = objs.Count
...
#Region "RADGRID EVENTS"
    Protected Sub rdgCategorieQuestion_ItemCommand(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridCommandEventArgs) Handles rdgCategorieQuestion.ItemCommand
        Try
            If e.CommandName = Telerik.Web.UI.RadGrid.ExportToExcelCommandName Then
                rdgCategorieQuestion.ExportSettings.ExportOnlyData = True
                rdgCategorieQuestion.GridLines = GridLines.Both
                rdgCategorieQuestion.ExportSettings.IgnorePaging = True
                rdgCategorieQuestion.ExportSettings.OpenInNewWindow = False
                rdgCategorieQuestion.ExportSettings.FileName = "Liste des CategorieQuestion"
                rdgCategorieQuestion.MasterTableView.Columns(0).Visible = False
                rdgCategorieQuestion.MasterTableView.ExportToExcel()
            End If

            Dim _id As Long = TypeSafeConversion.NullSafeLong(e.CommandArgument)
            Select Case e.CommandName
                Case "delete"
                    Dim obj As New Cls_CategorieQuestion(_id)
                    obj.Delete()
                    User_Connected.Activite_Utilisateur_InRezo("DELETE " & PAGE_TITLE, obj.LogData(obj), Request.UserHostAddress)
                    'User_Connected.Activite_Utilisateur_InRezo("DELETE CategorieQuestion ", obj.ID & " - Code:" & obj.Titrerapport & " Prop:", Request.UserHostAddress)
                    MessageToShow([Global].Msg_Information_Supprimee_Avec_Succes, "S")
                    rdgCategorieQuestion.Rebind()
            End Select
        Catch ex As Threading.ThreadAbortException
        Catch ex As Rezo509Exception
            MessageToShow(ex.Message)
    ...
    Protected Sub rdgCategorieQuestion_ItemDataBound(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridItemEventArgs) Handles rdgCategorieQuestion.ItemDataBound
        Try
            ...
            If e.Item.ItemType = GridItemType.Item Or e.Item.ItemType = GridItemType.AlternatingItem Then
                ...
                '_lnk.Attributes.Add("onclick", "javascript:void(ShowAddUpdateForm('Frm_CategorieQuestionADD.aspx?ID=" & CLng(_lbl_ID.Text) & "', 750, 400));")
            End If

            If (gridDataItem IsNot Nothing) Then
                Dim item As GridDataItem = gridDataItem
                CType(item.FindControl("lbOrder"), Label).Text = rdgCategorieQuestion.PageSize * rdgCategorieQuestion.CurrentPageIndex + (item.RowIndex / 2)
                ...
                imageediter.Attributes.Add("onclick", "javascript:void(ShowAddUpdateForm('Frm_CategorieQuestionADD.aspx?ID=" & CType(DataBinder.Eval(e.Item.DataItem, "ID"), Long) & "&" & [Global].ACTION & "=" & [Global].HideMenuHeader & "',900,650));")
                REM Privilege
    ...
    Protected Sub rdgCategorieQuestion_NeedDataSource(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles rdgCategorieQuestion.NeedDataSource
        If IsPostBack Then
            BindGrid(False)
        End If
    End Sub
    ...
    Protected Sub rbtnClearFilters_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles rbtnClearFilters.Click
        Try
            For Each column As GridColumn In rdgCategorieQuestion.MasterTableView.Columns
                column.CurrentFilterFunction = GridKnownFunction.NoFilter
                column.CurrentFilterValue = String.Empty
            Next
            rdgCategorieQuestion.MasterTableView.FilterExpression = String.Empty
            rdgCategorieQuestion.MasterTableView.Rebind()
        Catch ex As Threading.ThreadAbortException
        Catch ex As Rezo509Exception
            MessageToShow(ex.Message)
        Catch ex As Exception
