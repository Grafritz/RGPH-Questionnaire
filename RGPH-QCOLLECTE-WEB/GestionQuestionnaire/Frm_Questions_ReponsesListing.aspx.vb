...
Partial Class GestionQuestionnaire_Frm_Questions_ReponsesListing
...
#Region "Load DATA"
    ...
    Private Sub BindGrid(Optional ByVal _refresh As Boolean = True)
        ...
        Try
            objs = Cls_Questions_Reponses.SearchAll(IDRubrique)
...
#Region "RADGRID EVENTS"
    Protected Sub rdgQuestions_Reponses_ItemCommand(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridCommandEventArgs) Handles rdgQuestions_Reponses.ItemCommand
        Try
            If e.CommandName = Telerik.Web.UI.RadGrid.ExportToExcelCommandName Then
                rdgQuestions_Reponses.ExportSettings.ExportOnlyData = True
                rdgQuestions_Reponses.GridLines = GridLines.Both
                rdgQuestions_Reponses.ExportSettings.IgnorePaging = True
                rdgQuestions_Reponses.ExportSettings.OpenInNewWindow = False
                rdgQuestions_Reponses.ExportSettings.FileName = "Liste des Questions_Reponses"
                rdgQuestions_Reponses.MasterTableView.Columns(0).Visible = False
                rdgQuestions_Reponses.MasterTableView.ExportToExcel()
            End If
    ...
    Protected Sub rbtnClearFilters_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles rbtnClearFilters.Click
        Try
            For Each column As GridColumn In rdgQuestions_Reponses.MasterTableView.Columns
                column.CurrentFilterFunction = GridKnownFunction.NoFilter
                column.CurrentFilterValue = String.Empty
            Next
            rdgQuestions_Reponses.MasterTableView.FilterExpression = String.Empty
            rdgQuestions_Reponses.MasterTableView.Rebind()
        Catch ex As Threading.ThreadAbortException
        Catch ex As Rezo509Exception
            MessageToShow(ex.Message)
        Catch ex As Exception
