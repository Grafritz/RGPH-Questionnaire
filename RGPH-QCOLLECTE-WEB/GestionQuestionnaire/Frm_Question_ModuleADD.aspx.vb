...
REM Date:1/19/2018 12:26:55 PM
Imports Microsoft
Imports System.Data
...
Partial Class Frm_Question_ModuleADD
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Is_Acces_Page Then
            If Not IsPostBack Then
                ...
                'rbtnAddQuestion_Module.Attributes.Add("onclick", "javascript:void(ShowAddUpdateForm('Frm_Question_ModuleADD.aspx', 950, 650)); return false;")
                'BtnADDNew.Attributes.Add("onclick", "javascript:Open_Window('Frm_Question_ModuleADD.aspx', '_self',500,400); return false;") 
                LOAD_ALL_DATA()
            End If
        End If
    End Sub
...
#Region "Load DATA"
        ...
        Try
            If Request.QueryString("ID") IsNot Nothing Then
                ...
                If obj.ID > 0 Then
                    ...
                    With obj
                        DDL_CodeModule.SelectedIndex = DDL_CodeModule.Items.IndexOf(DDL_CodeModule.Items.FindByValue(.CodeModule))

                        CheckDebutChamps()
                        DDL_CodeQuestion.SelectedIndex = DDL_CodeQuestion.Items.IndexOf(DDL_CodeQuestion.Items.FindByValue(.CodeQuestion))
                        CB_EstDebut.Checked = .EstDebut
                    End With
                End If
            Else
    ...
    Private Sub FillCombo_CodeModule()
        Try
            Dim objs1 As List(Of Cls_Module) = Cls_Module.SearchAll_Actif
            With DDL_CodeModule
                .DataSource = objs1
    ...
    Private Sub FillCombo_CodeQuestion()
        Try
            Dim CodeModule As String = TypeSafeConversion.NullSafeString(DDL_CodeModule.SelectedValue)

            Dim objs1 As List(Of Cls_Questions) = Cls_Questions.SearchAll_NotIn_QuestionModule_ByIDModule(CodeModule)
...
#Region "METHODES - SAVE"
    Private Sub SAVE_QUESTION_MODULE(ByVal CloseWindow As Boolean)
        Try
            Dim _id As Long = TypeSafeConversion.NullSafeLong(txt_CodeQuestion_Module_Hid.Text)
            Dim obj As New Cls_Question_Module(_id)
            With obj
                .CodeModule = DDL_CodeModule.SelectedValue
                .CodeQuestion = DDL_CodeQuestion.SelectedValue
                .EstDebut = CB_EstDebut.Checked
            End With
            obj.Save(User_Connected.Username)
            REM TRACE UTILUSATEUR / Trace Transaction
            User_Connected.Activite_Utilisateur_InRezo(IIf(_id <= 0, "ADD ", "EDIT ") & " Question Module", obj.LogData(obj), Request.UserHostAddress)
            'txt_CodeQuestion_Module_Hid.Text = obj.ID
            '_message = "Sauvegarde Effectuée"
            MessageToShow([Global].Msg_Enregistrement_Effectue, "S", False)
            'RadAjaxManager1.ResponseScripts.Add("CloseAndRefreshListeQuestion_Module();")
            If CloseWindow Then
...
#Region "EVENTS BUTTON"
    ...
    Private Sub DDL_CodeModule_SelectedIndexChanged(sender As Object, e As EventArgs) Handles DDL_CodeModule.SelectedIndexChanged
        CheckDebutChamps()
    End Sub

    Private Sub CheckDebutChamps()
        Try
            FillCombo_CodeQuestion()
            Dim obj As New Cls_Question_Module(DDL_CodeModule.SelectedValue)
            With obj
                If .ID > 0 Then
                    DIV_EstDebut.Visible = False
                    CB_EstDebut.Checked = False
                Else
                    DIV_EstDebut.Visible = True
                End If
            End With
        Catch ex As Threading.ThreadAbortException
        Catch ex As Rezo509Exception
