...
Partial Class Frm_TypeQuestionListing
...
#Region "ATTRIBUTS"
    ...
    Private Const Nom_page As String = "PAGE-LISTING-TYPE-QUESTION"  ' POUR LA PAGE
    Private Const Btn_Save As String = "Bouton-SAVE-TYPE-QUESTION"       ' POUR LE BOUTON D'ENREGISTREMENT
    Private Const Btn_Edit As String = "Bouton-EDIT-TYPE-QUESTION"       ' POUR LE BOUTON DE MODIFICATION
    Private Const Btn_Delete As String = "Bouton-DELETE-TYPE-QUESTION"   ' POUR LE BOUTON DE SUPPRESSION
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Is_Acces_Page Then
            If Not IsPostBack Then
                ...
                Btn_ADD_TypeQuestion.Attributes.Add("onclick", "javascript:void(ShowAddUpdateForm('Frm_TypeQuestionADD.aspx?" & [Global].ACTION & "=" & [Global].HideMenuHeader & "', 950, 650)); return false;")
                'Btn_ADD_TypeQuestion.Attributes.Add("onclick", "javascript:Open_Window('Frm_TypeQuestionADD.aspx', '_self',500,400); return false;") 
                BindGrid()
            End If
        End If
    End Sub
...
#Region "SECURITE"
    Public Sub SYSTEME_SECURITE()
        Try
            ...
            'CType(Page.Master.FindControl("li_TypeQuestion"), HtmlControl).Attributes.Add("class", "active ")
            'CType(Page.Master.FindControl("i_TypeQuestion"), HtmlControl).Attributes.Add("class", "fa fa-folder-open fa-lg ")
            ...
            CType(Page.Master.FindControl("LI_PARAMETRES"), HtmlControl).Attributes.Add("class", "parent open")
            CType(Page.Master.FindControl("li_TypeQuestionListing"), HtmlControl).Attributes.Add("class", "active")
            ...
            If Session([Global].GLOBAL_SESSION) IsNot Nothing Then
                ...
                Else    ' SI L'UTILISATEUR A ACCES A LA PAGE ON VERIFIE POUR LES BOUTONS ET LES LIENS
                    ...
                    Btn_ADD_TypeQuestion.Visible = _check
                    rdgTypeQuestion.MasterTableView.Columns.FindByUniqueNameSafe("editer").Visible = _check
                    If Request.QueryString([Global].ACTION) IsNot Nothing Then
                    ...
                    rdgTypeQuestion.MasterTableView.Columns.FindByUniqueNameSafe("delete").Visible = _check
                End If
            End If
...
#Region "Load DATA"
    Private Sub BindGrid(Optional ByVal _refresh As Boolean = True)
        Dim objs As List(Of Cls_TypeQuestion)
        Dim _ret As Long = 0
        Try
            objs = Cls_TypeQuestion.SearchAll
            rdgTypeQuestion.DataSource = objs
            If _refresh Then
                rdgTypeQuestion.DataBind()
            End If
            _ret = objs.Count
...
#Region "RADGRID EVENTS"
    Protected Sub rdgTypeQuestion_ItemCommand(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridCommandEventArgs) Handles rdgTypeQuestion.ItemCommand
        Try
            If e.CommandName = Telerik.Web.UI.RadGrid.ExportToExcelCommandName Then
                rdgTypeQuestion.ExportSettings.ExportOnlyData = True
                rdgTypeQuestion.GridLines = GridLines.Both
                rdgTypeQuestion.ExportSettings.IgnorePaging = True
                rdgTypeQuestion.ExportSettings.OpenInNewWindow = False
                rdgTypeQuestion.ExportSettings.FileName = "Liste des TypeQuestion"
                rdgTypeQuestion.MasterTableView.Columns(0).Visible = False
                rdgTypeQuestion.MasterTableView.ExportToExcel()
            End If

            Dim _id As Long = TypeSafeConversion.NullSafeLong(e.CommandArgument)
            Select Case e.CommandName
                Case "delete"
                    Dim obj As New Cls_TypeQuestion(_id)
                    obj.Delete()
                    User_Connected.Activite_Utilisateur_InRezo("DELETE " & PAGE_TITLE, obj.LogData(obj), Request.UserHostAddress)
                    'User_Connected.Activite_Utilisateur_InRezo("DELETE TypeQuestion ", obj.ID & " - Code:" & obj.Titrerapport & " Prop:", Request.UserHostAddress)
                    MessageToShow([Global].Msg_Information_Supprimee_Avec_Succes, "S")
                    rdgTypeQuestion.Rebind()
            End Select
        Catch ex As Threading.ThreadAbortException
        Catch ex As Rezo509Exception
            MessageToShow(ex.Message)
    ...
    Protected Sub rdgTypeQuestion_ItemDataBound(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridItemEventArgs) Handles rdgTypeQuestion.ItemDataBound
        Try
            ...
            If e.Item.ItemType = GridItemType.Item Or e.Item.ItemType = GridItemType.AlternatingItem Then
                ...
                '_lnk.Attributes.Add("onclick", "javascript:void(ShowAddUpdateForm('Frm_TypeQuestionADD.aspx?ID=" & CLng(_lbl_ID.Text) & "', 750, 400));")
            End If

            If (gridDataItem IsNot Nothing) Then
                Dim item As GridDataItem = gridDataItem
                CType(item.FindControl("lbOrder"), Label).Text = rdgTypeQuestion.PageSize * rdgTypeQuestion.CurrentPageIndex + (item.RowIndex / 2)
                ...
                imageediter.Attributes.Add("onclick", "javascript:void(ShowAddUpdateForm('Frm_TypeQuestionADD.aspx?ID=" & CType(DataBinder.Eval(e.Item.DataItem, "ID"), Long) & "&" & [Global].ACTION & "=" & [Global].HideMenuHeader & "',900,650));")
                REM Privilege
    ...
    Protected Sub rdgTypeQuestion_NeedDataSource(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles rdgTypeQuestion.NeedDataSource
        If IsPostBack Then
            BindGrid(False)
        End If
    End Sub
    ...
    Protected Sub rbtnClearFilters_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles rbtnClearFilters.Click
        Try
            For Each column As GridColumn In rdgTypeQuestion.MasterTableView.Columns
                column.CurrentFilterFunction = GridKnownFunction.NoFilter
                column.CurrentFilterValue = String.Empty
            Next
            rdgTypeQuestion.MasterTableView.FilterExpression = String.Empty
            rdgTypeQuestion.MasterTableView.Rebind()
        Catch ex As Threading.ThreadAbortException
        Catch ex As Rezo509Exception
            MessageToShow(ex.Message)
        Catch ex As Exception
