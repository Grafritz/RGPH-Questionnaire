...
REM Date:1/18/2018 11:46:31 AM
Imports Microsoft
Imports System.Data
...
Partial Class Frm_CategorieQuestionADD
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Is_Acces_Page Then
            If Not IsPostBack Then
                ...
                'rbtnAddCategorieQuestion.Attributes.Add("onclick", "javascript:void(ShowAddUpdateForm('Frm_CategorieQuestionADD.aspx', 950, 650)); return false;")
                'BtnADDNew.Attributes.Add("onclick", "javascript:Open_Window('Frm_CategorieQuestionADD.aspx', '_self',500,400); return false;") 
                LOAD_ALL_DATA()
            End If
        End If
    End Sub
...
#Region "Load DATA"
    Private Sub LOAD_ALL_DATA()
        LOAD_CATEGORIEQUESTION()
    End Sub

    Private Sub LOAD_CATEGORIEQUESTION()
        Try
            If Request.QueryString("ID") IsNot Nothing Then
                Dim _id As Long = TypeSafeConversion.NullSafeLong(Request.QueryString("ID"))
                txt_CodeCategorieQuestion_Hid.Text = _id
                ...
                If obj.ID > 0 Then
                    ...
                    With obj
                        txt_CodeCategorie.Text = .CodeCategorie
                        txt_CategorieQuestion.Text = .CategorieQuestion
                        txt_DetailsCategorie.Text = .DetailsCategorie
                        txt_SousDetailsCategorie.Text = .SousDetailsCategorie
                    End With
                End If
            Else
...
#Region "METHODES - SAVE"
    Private Sub SAVE_CATEGORIEQUESTION()
        Try
            Dim _id As Long = TypeSafeConversion.NullSafeLong(txt_CodeCategorieQuestion_Hid.Text)
            Dim obj As New Cls_CategorieQuestion(_id)
            With obj
                .CodeCategorie = txt_CodeCategorie.Text
                .CategorieQuestion = txt_CategorieQuestion.Text
                .DetailsCategorie = txt_DetailsCategorie.Text
                .SousDetailsCategorie = txt_SousDetailsCategorie.Text
            End With
            obj.Save(User_Connected.Username)
            REM TRACE UTILUSATEUR / Trace Transaction
            User_Connected.Activite_Utilisateur_InRezo(IIf(_id <= 0, "ADD ", "EDIT ") & " CategorieQuestion", obj.LogData(obj), Request.UserHostAddress)
            txt_CodeCategorieQuestion_Hid.Text = obj.ID
            '_message = "Sauvegarde Effectuée"
            MessageToShow([Global].Msg_Enregistrement_Effectue, "S", False)
            'RadAjaxManager1.ResponseScripts.Add("CloseAndRefreshListeCategorieQuestion();")
