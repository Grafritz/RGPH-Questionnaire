...
Partial Class Frm_SR_SERVICESListing
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Is_Acces_Page Then
            If Not IsPostBack Then
                rbtnAddSR_SERVICES.Attributes.Add("onclick", "javascript:void(ShowAddUpdateForm('Frm_SR_SERVICESADD.aspx', 750, 400)); return false;")
    ...
    Public Sub SYSTEME_SECURITE()
        ...
        If Session([Global].GLOBAL_SESSION) IsNot Nothing Then
            ...
            Else    ' SI L'UTILISATEUR A ACCES A LA PAGE ON VERIFIE POUR LES BOUTONS ET LES LIENS
                ...
                rbtnAddSR_SERVICES.Visible = Cls_Privilege.VerifyRightOnObject(Btn_Save, User_Connected.IdGroupeuser)
            End If
        End If
...
#Region "Load DATA"
    Private Sub BindGrid(Optional ByVal _refresh As Boolean = True)
        'Dim objs As List(Of Cls_Privilege)
        Dim _ret As Long = 0
        Try
            'objs = Cls_Privilege.SearchAll(True)
            'rdgSR_SERVICES.DataSource = objs
            'If _refresh Then
            '    rdgSR_SERVICES.DataBind()
            'End If
            '_ret = objs.Count
            LabelTitre.Text = "SERVICES <small class=""badge  bg-blue"">" & _ret & "</small>"
        Catch ex As Rezo509Exception
            _message = ex.Message
            MessageToShow(_message)
...
#Region "RADGRID EVENTS"
    Protected Sub rdgSR_SERVICES_ItemCommand(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridCommandEventArgs) Handles rdgSR_SERVICES.ItemCommand
        Try
            If e.CommandName = Telerik.Web.UI.RadGrid.ExportToExcelCommandName Then
                rdgSR_SERVICES.ExportSettings.ExportOnlyData = True
                rdgSR_SERVICES.GridLines = GridLines.Both
                rdgSR_SERVICES.ExportSettings.IgnorePaging = True
                rdgSR_SERVICES.ExportSettings.OpenInNewWindow = False
                rdgSR_SERVICES.ExportSettings.FileName = "Liste des SR_SERVICES"
                rdgSR_SERVICES.MasterTableView.Columns(0).Visible = False
                rdgSR_SERVICES.MasterTableView.ExportToExcel()
            End If

            Select Case e.CommandName
                Case "delete"
                    Dim obj As New Cls_Privilege(TypeSafeConversion.NullSafeLong(e.CommandArgument))
                    obj.Delete()
                    User_Connected.Activite_Utilisateur_InRezo("DELETE PRODUITS/SERVICES REZO509 ", obj.ID & " - NomService:" & obj.NOM_OBJET & " Prop:", Request.UserHostAddress)
                    MessageToShow("Service  supprimé", "S")
                    rdgSR_SERVICES.Rebind()
            End Select
        Catch ex As Rezo509Exception
            _message = ex.Message
            MessageToShow(_message)
            _message = " Methode -> [ rdgSR_SERVICES_ItemCommand ] :" & ex.Message
            [Global].WriteError(_message, User_Connected)
        End Try
    End Sub

    Protected Sub rdgSR_SERVICES_ItemDataBound(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridItemEventArgs) Handles rdgSR_SERVICES.ItemDataBound
        Try
            ...
            If e.Item.ItemType = GridItemType.Item Or e.Item.ItemType = GridItemType.AlternatingItem Then
                ...
                '_lnk.Attributes.Add("onclick", "javascript:void(ShowAddUpdateForm('Frm_SR_SERVICESADD.aspx?ID=" & CLng(_lbl_ID.Text) & "', 750, 400));")
            End If

            If (gridDataItem IsNot Nothing) Then
                Dim item As GridDataItem = gridDataItem
                CType(item.FindControl("lbOrder"), Label).Text = rdgSR_SERVICES.PageSize * rdgSR_SERVICES.CurrentPageIndex + (item.RowIndex / 2)
                ...
                imagedelete.ToolTip = "Effacer ce services"
                imageediter.ToolTip = "Editer ce services"
                imagedelete.CommandArgument = CType(DataBinder.Eval(e.Item.DataItem, "ID"), String)
                imageediter.Attributes.Add("onclick", "javascript:void(ShowAddUpdateForm('Frm_SR_SERVICESADD.aspx?ID=" & CType(DataBinder.Eval(e.Item.DataItem, "ID"), Long) & "',800,550));")
                REM Privilege
        ...
        Catch ex As Rezo509Exception
            _message = ex.Message
            MessageToShow(_message)
            _message = " Methode -> [ rdgSR_SERVICES_ItemDataBound ] :" & ex.Message
            [Global].WriteError(_message, User_Connected)
        End Try
    End Sub

    Protected Sub rdgSR_SERVICES_NeedDataSource(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles rdgSR_SERVICES.NeedDataSource
        If IsPostBack Then
            BindGrid(False)
        End If
    End Sub
